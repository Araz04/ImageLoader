...
/**
 * A lightweight image loading and caching library built without any external image libraries.
 *
 * Features:
 * - Two-level caching: in-memory [MemoryCache] + [DiskCache]
 * - 4-hour TTL on all cached images
 * - Coroutine-based async loading
 * - Placeholder support while loading
 * - Manual cache invalidation (single URL or full clear)
 * - Cancellable per-view loads (prevents recycled-view flicker)
 *
 * Usage (Kotlin):
 * ```kotlin
 * ImageLoader.getInstance(context)
 *     .load(imageView, "https://example.com/photo.jpg", R.drawable.placeholder)
 * ```
 *
 * Usage (Java):
 * ```java
